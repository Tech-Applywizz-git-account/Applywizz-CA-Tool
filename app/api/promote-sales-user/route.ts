...
export async function POST(req: Request) {
    try {
        ...
        // Validate Sales Head promotion path: only SBDA can be promoted
        if (newRole === "Sales Head") {
            const currentActiveRole = user.role;
            const historyKeys = Object.keys(user.role_history || {}).sort();
            const latestHistoryRole = historyKeys.length > 0 ? (user.role_history || {})[historyKeys[historyKeys.length - 1]] : currentActiveRole;
            const effectiveRole = latestHistoryRole || currentActiveRole;
            if (effectiveRole !== "SBDA") {
                return NextResponse.json({ 
                    success: false, 
                    error: "Only SBDA representatives can be promoted to Sales Head" 
                }, { status: 400 });
            }
        }
