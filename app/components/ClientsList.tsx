...
export default function ClientsList({
  ...
  const fetchClients = useCallback(async () => {
    ...
    const urlFrom = searchParams ? searchParams.get("from") : null
    const urlTo = searchParams ? searchParams.get("to") : null
    const today = new Date().toISOString().split("T")[0]
    const isHistorical = urlFrom && urlTo && (urlFrom !== today || urlTo !== today)

    if (isHistorical) {
      // 1. Fetch CAs if filtering by team
      let caIdsToFilter: string[] | null = null
      if ((teamIds && teamIds.length > 0) || teamId || assignedCAId) {
        let caQuery = supabase.from("users").select("id")
        if (teamIds && teamIds.length > 0) caQuery = caQuery.in("team_id", teamIds)
        else if (teamId) caQuery = caQuery.eq("team_id", teamId)

        const { data: caData } = await caQuery
        if (caData) {
          caIdsToFilter = caData.map(c => c.id)
          if (assignedCAId) {
            caIdsToFilter = caIdsToFilter.filter(id => id === assignedCAId)
          }
        }
      } else if (assignedCAId) {
        caIdsToFilter = [assignedCAId]
      }

      // 2. Fetch work history
      let whQuery = supabase.from("work_history").select("completed_profiles, date, ca_name, ca_id").gte("date", urlFrom).lte("date", urlTo)
      if (caIdsToFilter && caIdsToFilter.length > 0) {
        whQuery = whQuery.in("ca_id", caIdsToFilter)
      }

      const { data: whData, error } = await whQuery
      if (error) {
        setClients([])
        setTotal(0)
        setLoading(false)
      ...
      let histClients: Client[] = []
      const seenIds = new Set<string>() // prevent duplicate ids in memory list

      whData?.forEach(row => {
        if (Array.isArray(row.completed_profiles)) {
          row.completed_profiles.forEach(p => {
            histClients.push({
              ...p,
              id: `${p.id}-${row.date}`, // ensure unique keys for history logs
              assigned_ca_name: p.assigned_ca_name || row.ca_name,
              work_done_user: { id: row.ca_id, name: row.ca_name }
            })
            seenIds.add(p.id)
          })
        }
      ...
      let filtered = histClients
      if (status !== "all") {
        filtered = filtered.filter(c => c.status === status)
      }
      if (active !== "all") {
        const wantActive = active === "active"
        filtered = filtered.filter(c => wantActive ? (c.is_active !== false && c.status !== "Paused") : (c.is_active === false || c.status === "Paused"))
      }
      ...
      filtered.sort((a, b) => {
        let valA = a[sortKey as keyof Client]
        let valB = b[sortKey as keyof Client]
        if (valA == null) valA = ""
        if (valB == null) valB = ""
      ...
      setClients(filtered.slice(fromRange, toRange + 1) as Client[])
      setLoading(false)
      return
    }
    ...
    setLoading(false)
  }, [teamId, teamIds, assignedCAId, status, active, sortKey, sortDir, fromRange, toRange, q, pageSize, searchParams])
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-end gap-3">
        <span className="text-sm text-slate-600">
          {fromRange + 1}-{Math.min(toRange + 1, total)} of {total}
        </span>
        <Button
          variant="outline"
          size="sm"
