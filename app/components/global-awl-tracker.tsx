...
export function GlobalAwlTracker({ monthName, targetDate, salesReps = [], basePath = "/cro-dashboard" }: GlobalAwlTrackerProps) {
      ...
      predictedIdsData.forEach(({ id, expected }: { id: string, expected: number }) => {
        ...
        if (!recordsMap[id]) {
          recordsMap[id] = {
            ...
            repName,
            repEmail,
            shiftDate, // Initial prediction date
            type: 'predicted',
            matched: false,
            matchedDate: null,
            expected,
            ...
            // If already exists, maybe update expected if this one is higher?
            // Usually one prediction per month, but let's be safe.
            if (expected > recordsMap[id].expected) recordsMap[id].expected = expected;
        }
      });

      // 2. Process Matches
      matchedIds.forEach((id: string) => {
        if (!id || id === 'NULL' || id === 'UNDEFINED') return;
        ...
        if (!recordsMap[id]) {
          ...
          recordsMap[id] = {
            ...
            type: 'unpredicted',
            matched: true,
            matchedDate: shiftDate,
            expected: 0,
            actual: actualSalesMap[id] || 0
          };
        } else {
          // Predicted sale that is now matched
          recordsMap[id].matched = true;
          recordsMap[id].matchedDate = shiftDate;
          if (actualSalesMap[id]) {
            recordsMap[id].actual = actualSalesMap[id];
          } else if (recordsMap[id].expected > 0 && recordsMap[id].actual === 0) {
            recordsMap[id].actual = recordsMap[id].expected;
          }
        }
      });
    });
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchRep, searchAwl, statusFilter, startDate, endDate]);
