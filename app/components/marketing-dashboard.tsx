...
export function MarketingDashboard({ user, onLogout, viewerMode }: MarketingDashboardProps) {
    ...
    const totalSalesPages = Math.ceil(freshSales.length / itemsPerPage) || 1;
    const paginatedSales = freshSales.slice((salesPage - 1) * itemsPerPage, salesPage * itemsPerPage);
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <Card className="hover:shadow-md transition-shadow border-l-4 border-l-blue-500">
                                ...
                                <CardContent className="pt-4 space-y-2">
                                    <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                                        <span className="text-slate-600 font-medium">Apply-Wizz:</span>
                                        <span className="font-bold text-slate-800">{incentiveData?.total_fresh_sales || 0} Leads</span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                                    ...
                                    <div className="flex justify-between items-center text-sm border-t border-slate-100 pt-2">
                                        <span className="text-slate-600 font-medium">From Influencer:</span>
                            ...
                            <Card className="hover:shadow-md transition-shadow border-l-4 border-l-emerald-500 relative overflow-hidden">
                                ...
                                <CardContent className="pt-4 space-y-2">
                                    <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                                        <span className="text-slate-600 font-medium">Apply-Wizz:</span>
                                        <span className="font-bold text-slate-800">{showIncentives ? `₹${(incentiveData?.applywizz_incentive_inr || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '****'}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                                        <span className="text-slate-600 font-medium">Job Board:</span>
                        ...
                        <Card className="shadow-sm border-slate-200 mt-8">
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <CardTitle className="text-lg">Verified Fresh Sales Leads ({getMonthName()})</CardTitle>
                                <div className="text-sm text-slate-500 pt-1">Total: {freshSales.length} Leads</div>
                            </CardHeader>
                            <CardContent>
                                {freshSales.length === 0 ? (
                                    <div className="text-center py-8 text-slate-500">No fresh sales accurately recorded for this month.</div>
                                ) : (
                                            ...
                                            <Table>
                                                <TableHeader>
                                                    <TableRow className="bg-slate-50 hover:bg-slate-50">
                                                        ...
                                                        <TableHead className="font-semibold">Initial Closing Date</TableHead>
                                                ...
                                                <TableBody>
                                                    {paginatedSales.map((sale: any, i: number) => (
                                                        <TableRow key={sale.id || sale.lead_id || `fresh-${i}`} className="hover:bg-indigo-50/50 transition-colors">
                                                            ...
                                                            <TableCell>
                                                                <Badge className={
                                                                    ...
                                                                    {sale.finance_status || "Unknown"}
                                                                </Badge>
                                                            </TableCell>
                                                            <TableCell className="text-right">
                                                                <Badge className="bg-emerald-100 text-emerald-800 border-none font-bold">+$3.00 to Pool</Badge>
                                                            </TableCell>
                                                        </TableRow>
                                                    ))}
                                                </TableBody>
                                        ...
                                        {totalSalesPages > 1 && (
                                            <div className="flex items-center justify-between">
                                                <div className="text-sm text-slate-500">
                                                    Showing {(salesPage - 1) * itemsPerPage + 1} to {Math.min(salesPage * itemsPerPage, freshSales.length)} of {freshSales.length} entries
                                                </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="bg-emerald-50 rounded-xl p-5 border border-emerald-100 shadow-sm relative overflow-hidden">
                                            ...
                                            <h3 className="font-bold text-emerald-900 mb-2 flex items-center gap-2">
                                                <CheckCircle2 className="h-5 w-5 text-emerald-600" /> 1. The Apply-Wizz Webpage Target
                                            </h3>
                                            <p className="text-sm text-emerald-800">
                                                Every single verified, successfully <strong>closed sale</strong> originating as a uniquely new lead attributes precisely <strong>$3 USD</strong> into the unified Global Marketing Pool.
                                            </p>
                                            <div className="mt-3 bg-white p-3 rounded-md text-xs border border-emerald-200 text-slate-600 italic">
                                                <strong>Rule:</strong> The entire accumulated USD pool is split absolutely evenly down the middle amongst all authorized Marketing team members active during that month. Renwals explicitly do not count.
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-pink-50 rounded-xl p-5 border border-pink-100 shadow-sm relative overflow-hidden">
                                            ...
                                            <h3 className="font-bold text-pink-800 mb-2 flex items-center justify-between">
                                                <span>4. From Influencer Targets</span>
